...
proc mmap(address: pointer, length: int, prot: int, flags: int, fd: int, offset: int): ptr uint8 {.importc, varargs, header: "sys/mman.h".}
...
proc compileAndMap(blocks: seq[AdditionAndMoveBlock]): proc(): int {.cdecl.} =
    ...
    let code_block = (ptr uint8) mmap(nil, MAX_CODE_SIZE, 7, 0x22, -1, 0)
    let data_block = (ptr uint8) mmap(nil, MAX_DATA_SIZE, 3, 0x22, -1, 0)
    # dd code_block.repr
    # dd data_block.repr
    # The addresses seem close, that's probably good for performance!
